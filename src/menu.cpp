...
void menu(RenderWindow& window)
{
    ...
    while (svit == 1) {
        ...
        text2.setString(
                ...
                "generator.\n Control is carried out by pressing the LMB on "
                "the "
                "chase with the command that you want to advance further.\nIn "
                "order to advance the teams to the next round, it is necessary "
                "to "
                "press the enter key.\nThe menu is returned by pressing the "
                "ESC "
                "key.\n \n");
        text3.setString(
        ...
        while (isMenu) {
            ...
            if (IntRect(620, 200, 125, 50)
                        .contains(Mouse::getPosition(window))) {
                menu1.setColor(Color::Blue);
                menuNum = 1;
            }
            if (IntRect(620, 250, 130, 50)
                        .contains(Mouse::getPosition(window))) {
                menu2.setColor(Color::Blue);
                menuNum = 2;
            }
            if (IntRect(630, 350, 110, 50)
                        .contains(Mouse::getPosition(window))) {
    ...
    while (svit == 3) {
        ...
        Text congrats("Congrats!", font, 80);
